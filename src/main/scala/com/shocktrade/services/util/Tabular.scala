...
class Tabular() {
  ...
  private def convert[A](headers: Seq[String], v: A): Map[String, String] = {
    val beanClass = v.getClass
    Map(headers map (f => (f, asString(invokeMethod(v, f)))): _*)
  }

  private def invokeMethod[A](v: A, f: String) = {
  ...
  private def getHeaders[A](value: A): Seq[String] = {
   ...
   */
  private def unwantedFields(s: String) = Set("$outer").contains(s)
 ...
 */
object Tabular {
  private[this] val PRIMITIVES = Set[Class[_]](
